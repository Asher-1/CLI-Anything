.PHONY: help sync sync-merge sync-ours sync-adapt sync-adapt-only verify-fork sync-dry-run check-upstream show-diff backup restore clean customize customize-preview
...
help:
	...
	@echo "  make sync-adapt        - Sync upstream (rebase) + fork adaptation (recommended)"
	@echo "  make sync-adapt-only   - Run fork adaptation on current tree"
	@echo "  make verify-fork       - Verify no upstream hub URLs remain"
	@echo "  make sync              - Sync upstream (rebase strategy, legacy)"
...
# Recommended sync + adaptation workflow
sync-adapt:
	@echo "🚀 Starting upstream sync + fork adaptation (rebase)..."
	@./scripts/sync-upstream-adapt.sh --yes

sync-adapt-only:
	@./scripts/sync-upstream-adapt.sh --adapt-only
...
check-upstream:
	...
	@git fetch upstream --quiet 2>/dev/null || (git remote add upstream https://github.com/Asher-1/CLI-Anything.git && git fetch upstream --quiet)
