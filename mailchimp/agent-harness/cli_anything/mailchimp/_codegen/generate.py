...
def _generate_command_func(
    ...
    lines.append("        params.update(_json.loads(extra_params))")

    # Parse body
    if body_param is not None:
        lines.append("    body = _json.loads(data) if data else None")
...
def _generate_module(tag: str, operations: list[tuple[str, str, dict]]) -> str:
    ...
    # M4: no dead USE_JSON import at module top
    header = textwrap.dedent(f'''\
        """Commands for the Mailchimp /{group_name} API resource.
        ...
        def {group_var}():
            """{_slugify(tag)} resource commands."""
